/*  $Id: seqdbvol.cpp 351397 2012-01-27 20:05:15Z rafanovi $
 * ===========================================================================
 *
...
static char const rcsid[] = "$Id: seqdbvol.cpp 351397 2012-01-27 20:05:15Z rafanovi $";
...
static vector<Uint1>
	s_SeqDBMapNA2ToNA8Setup_v1()
{
	// Builds a table; each two bit slice holds 0,1,2 or 3.  These are
	// converted to whole bytes containing 1,2,4, or 8, respectively.

	vector<Uint1> translated;
	translated.reserve(65536*8);
	...
	for(int i = 0; i<65536; i++) {
		...
		translated.push_back(1 << p5);
		translated.push_back(1 << p6);
		translated.push_back(1 << p7);
		translated.push_back(1 << p8);
		translated.push_back(1 << p1);
		translated.push_back(1 << p2);
		translated.push_back(1 << p3);
		translated.push_back(1 << p4);
...
//////////////////////////////////////////////////////////////////////////
// added by kyzhao for gpu blastn
//typedef union __declspec(intrin_type) _CRT_ALIGN(8) __mint{
//	Uint4 uint4;
//	Uint1 uint1[4];
//};

//bool expanded_table_inited = false;
static vector<Uint1> expanded = s_SeqDBMapNA2ToNA8Setup();


//int total_translate = 0;
                   ...
                   const SSeqDBSlice & range)
{
    ...
    p = whole_chars_begin;
    //////////////////////////////////////////////////////////////////////////
    //added by kyzhao for gpu blastn testing
	//printf("loop times 1:%d\n", whole_chars_end - p);
#if OPT_TRACEBACK
#if 1
	...
	const Uint4* p_expanded = (Uint4*)&expanded.front();
	Uint4* 	p_buf8bit = (Uint4*)buf8bit;
	const unsigned char* p_buf2bit = (unsigned char*)buf2bit;
	Uint4 p_pos = pos/4;
	pos += (whole_chars_end-p)*4;
   	for(; p < whole_chars_end; )
	{
		Int4 table_offset = p_buf2bit[p++];//Int4 table_offset = (buf2bit[p] & 0xFF);
		p_buf8bit[p_pos++] = p_expanded[table_offset];
		//p_buf8bit[p_pos++] = p_expanded[p_buf2bit[p++]];
	}	
#endif
	...
	const Uint4* p_expanded = (Uint4*)&expanded.front();
	Uint4* 	p_buf8bit = (Uint4*)(buf8bit+pos);
	const __mint* p_buf2bit = (__mint*)(buf2bit+p);

	int loop_times = (whole_chars_end-p)/4;
	int rem = (whole_chars_end-p)%4;

	int buf8bit_offset = 0;
	__mint table_offset;
	for(int i = 0; i < loop_times; i++)
	{
		table_offset = p_buf2bit[i];//Int4 table_offset = (buf2bit[p] & 0xFF);
		buf8bit_offset = i<<2;

		p_buf8bit[buf8bit_offset++] = p_expanded[table_offset.uint1[0]];
		p_buf8bit[buf8bit_offset++] = p_expanded[table_offset.uint1[1]];
		p_buf8bit[buf8bit_offset++] = p_expanded[table_offset.uint1[2]];
		p_buf8bit[buf8bit_offset] = p_expanded[table_offset.uint1[3]];
	}
	if (rem > 0)
	{
		const unsigned char* pu_buf2bit = (unsigned char*) (buf2bit+whole_chars_end -rem);
		Uint4* 	p_buf8bit_1 = (Uint4*)(buf8bit+pos);
		p_buf8bit_1 += whole_chars_end-p-rem;
		for (int i = 0; i < rem; i++)
		{
			Int4 table_offset = pu_buf2bit[i];//Int4 table_offset = (buf2bit[p] & 0xFF);
			p_buf8bit_1[i] = p_expanded[table_offset];
		}
	}
...
#if 0
	//////////////////////////////////////////////////////////////////////////
	//added by kyzhao for gpu blastn SSE & AVX implementation
	__m128i mask_128i;// = {0x0C030C03,0x0C030C03,0x0C030C03,0x0C030C03};
	//mask_128i.m128i_u32[0] = 0x0C030C03;
	//mask_128i.m128i_u32[1] = 0x0C030C03;
	//mask_128i.m128i_u32[2] = 0x0C030C03;
	//mask_128i.m128i_u32[3] = 0x0C030C03;
	mask_128i.m128i_u64[0] = 0x030C030C030C030C;
	mask_128i.m128i_u64[1] = 0x030C030C030C030C;
	__m128i expanded_128i;// ={};
	//expanded_128i.m128i_u8[0] = 1;
	//expanded_128i.m128i_u8[1] = 2;
	//expanded_128i.m128i_u8[2] = 4;
	//expanded_128i.m128i_u8[3] = 8;
	//expanded_128i.m128i_u8[4] = 2;
	//expanded_128i.m128i_u8[5] = 0;
	//expanded_128i.m128i_u8[6] = 0;
	//expanded_128i.m128i_u8[7] = 0;
	//expanded_128i.m128i_u8[8] = 4;
	//expanded_128i.m128i_u8[9] = 0;
	//expanded_128i.m128i_u8[10] = 0;
	//expanded_128i.m128i_u8[11] = 0;
	//expanded_128i.m128i_u8[12] = 8;
	//expanded_128i.m128i_u8[13] = 0;
	//expanded_128i.m128i_u8[14] = 0;
	//expanded_128i.m128i_u8[15] = 0;
	expanded_128i.m128i_u64[0] = 0x0000000208040201;
	expanded_128i.m128i_u64[1] = 0x0000000800000004;

	const char* p_buf2bit = buf2bit+p;
	int loop_times_char = (whole_chars_end-p);
	int loop_times_int = loop_times_char/4;
	Uint4* p_buf2bit_int = (Uint4*) p_buf2bit;

	char* p_buf8bit = buf8bit+ pos;
	__m128i* p_buf8bit_128i = (__m128i*)p_buf8bit;

	char t_buf2bit_char[8] = {0};
	for (int i = 0; i < loop_times_int; i++)
	{
		char* p_buf2bit_char = (char*)&p_buf2bit_int[i];
		t_buf2bit_char[0] = p_buf2bit_char[0] >>4;
		t_buf2bit_char[1] = p_buf2bit_char[1] >>4;
		t_buf2bit_char[2] = p_buf2bit_char[2] >>4;
		t_buf2bit_char[3] = p_buf2bit_char[3] >>4;


		//__m128i t_ret = _mm_set_epi8(
		//	p_buf2bit_char[3], p_buf2bit_char[3], t_buf2bit_char[3], t_buf2bit_char[3],
		//	p_buf2bit_char[2], p_buf2bit_char[2], t_buf2bit_char[2], t_buf2bit_char[2],
		//	p_buf2bit_char[1], p_buf2bit_char[1], t_buf2bit_char[1], t_buf2bit_char[1],
		//	p_buf2bit_char[0], p_buf2bit_char[0], t_buf2bit_char[0], t_buf2bit_char[0]);

		__m128i t_ret;
		t_ret.m128i_u8[0] = t_buf2bit_char[0];
		t_ret.m128i_u8[1] = t_buf2bit_char[0];
		t_ret.m128i_u8[2] = p_buf2bit_char[0];
		t_ret.m128i_u8[3] = p_buf2bit_char[0];

		t_ret.m128i_u8[4] = t_buf2bit_char[1];
		t_ret.m128i_u8[5] = t_buf2bit_char[1];
		t_ret.m128i_u8[6] = p_buf2bit_char[1];
		t_ret.m128i_u8[7] = p_buf2bit_char[1];

		t_ret.m128i_u8[8] = t_buf2bit_char[2];
		t_ret.m128i_u8[9] = t_buf2bit_char[2];
		t_ret.m128i_u8[10] = p_buf2bit_char[2];
		t_ret.m128i_u8[11] = p_buf2bit_char[2];

		t_ret.m128i_u8[12] = t_buf2bit_char[3];
		t_ret.m128i_u8[13] = t_buf2bit_char[3];
		t_ret.m128i_u8[14] = p_buf2bit_char[3];
		t_ret.m128i_u8[15] = p_buf2bit_char[3];


		t_ret = _mm_and_si128(t_ret, mask_128i);
		t_ret = _mm_shuffle_epi8(expanded_128i,t_ret);
		_mm_storeu_si128(p_buf8bit_128i, t_ret);
		p_buf8bit_128i++;
	}

	int rem_loop_times_128i = loop_times_char%4;
	if (rem_loop_times_128i > 0)
	{
		const Uint4* p_expanded = (Uint4*)&expanded.front();
		Uint4* 	p_buf8bit = (Uint4*)buf8bit;
		p_buf8bit += loop_times_int*4;
		p += loop_times_int*4; 

		for(; p < whole_chars_end; p++)
		{
			Int4 table_offset = (buf2bit[p] & 0xFF);
			p_buf8bit[0] = p_expanded[table_offset];
			p_buf8bit++;
		}
	}
	pos += (whole_chars_end-p)*4;
...
#endif
#if 0
	static vector<Uint1> expanded_v1 = s_SeqDBMapNA2ToNA8Setup_v1();
	const Uint8* p_expanded_v1 = (Uint8*)&expanded_v1.front();
	Uint8* 	p_buf8bit = (Uint8*)(buf8bit+pos);
	Uint2* p_buf2bit = (Uint2*)(buf2bit+p);
	int looptimes = (whole_chars_end-p)/2;
	int rem_ = (whole_chars_end-p)%2;
	
	__m64 t_expanded;
	for(int i = 0;  i < looptimes; i++)
	{
		Int4 table_offset = (p_buf2bit[i] & 0xFFFF);
		t_expanded.m64_u64 = p_expanded_v1[table_offset];
		
		//p_buf8bit[i] = x;
		_mm_stream_pi((__m64*)(p_buf8bit+i),t_expanded);
	}
	if (rem_ >0)
	{
		const Uint8* p_expanded = (Uint8*)&expanded.front();
		Int4 table_offset = (buf2bit[whole_chars_end] & 0xFF);
		Uint4 x = p_expanded[table_offset];

		Uint4*p_buf8bit_1 = (Uint4*) (buf8bit+pos);
		p_buf8bit_1[whole_chars_end-p] = x;
	}

	//{
	//	Uint4* p_buf8bit_t = (Uint4*) (buf8bit+pos);
	//	for (int i = 0; i < whole_chars_end-p; i++)
	//	{
	//		printf("%x ",p_buf8bit_t[i]);
	//	}
	//	
	//}
...
#if	ONE_VOL
//////////////////////////////////////////////////////////////////////////
// added by kyzhao for gpu 2011.11.8
int CSeqDBVol::x_GetAllSequence(const char    ** buffer,
                             bool             keep,
                             CSeqDBLockHold & locked,
                             bool             can_release,
                             bool             in_lease) const
{
	...
	int lastOid = m_Idx->GetNumOIDs()-1;

	//Uint8 vol_total_len = m_Idx->GetVolumeLength();

	//TIndx start_offset_end = 0;
	//TIndx end_offset_end   = 0;
	//m_Idx->GetHdrStartEnd(0, start_offset_end,end_offset_end);
#if 0
	for (int i = 0; i< lastOid; i++)
	{													
		m_Idx->GetSeqStartEnd(i, start_offset, end_offset);
    ...
    m_Idx->GetSeqStartEnd(lastOid, start_offset, end_offset);
...
void CSeqDBVol::GetSeqStartEnd(int oid, TIndx& start_offset, TIndx& end_offset, CSeqDBLockHold & locked) const
{	
	m_Atlas.Lock(locked);
	if (!m_SeqFileOpened) x_OpenSeqFile(locked);
