...
public:
    ...
    /// members!
    ~CBlastOptionsMemento() {free(m_GpuOpts);}
...
private:
    CBlastOptionsMemento(CBlastOptionsLocal* local_opts)
    {
		...
		m_GpuOpts->query_list		= local_opts->GetQueryList().c_str();
		m_GpuOpts->use_gpu          = local_opts->GetUseGpu();             
		m_GpuOpts->gpu_id			= local_opts->GetGpuID();          
		m_GpuOpts->prepare_num      = local_opts->GetPrepareNum();         
		m_GpuOpts->prelim_num		= local_opts->GetPrelimNum();    
		m_GpuOpts->trace_num			= local_opts->GetTraceNum();        
		m_GpuOpts->print_num			= local_opts->GetPrintNum();  
		/* ********** FINISH ************* */
