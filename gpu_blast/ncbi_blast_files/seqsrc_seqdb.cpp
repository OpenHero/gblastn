/*  $Id: seqsrc_seqdb.cpp 351397 2012-01-27 20:05:15Z rafanovi $
* ===========================================================================
*
...
//////////////////////////////////////////////////////////////////////////
//added by kyzhao for gpu 2012.11.9
...
s_SeqDbGetSequence(void* seqdb_handle, BlastSeqSrcGetSeqArg* args)
{
    ...
    len = (datap->copied) 
        ...
        :  seqdb.GetSequence(oid, &buf); // modified by kyzhao for gpu 2012.11.8
		//:  seqdb.GetAllSequence(&buf);
...
/// @return return codes defined in blast_seqsrc.h
static Int2 
s_SeqDbGetAllSequence(void* seqdb_handle, BlastSeqSrcGetSeqArg* args)
{
    Int4 vol_oid = -1, oid = -1, len = 0;
    Boolean has_sentinel_byte;
    ...
    //if (args->check_oid_exclusion) {	// args->check_oid_exclusion = 0;
    //    if (! seqdb.GetIdSet().Blank()) {
    //        list< CRef<CSeq_id> > seqids = seqdb.GetSeqIDs(oid);
    //        
    //        if (seqids.empty()) {
...
static void
	s_SeqDbReleaseAllSequence(void* seqdb_handle, BlastSeqSrcGetSeqArg* args)
{
	TSeqDBData * datap = (TSeqDBData *) seqdb_handle;
...
//////////////////////////////////////////////////////////////////////////
// added by kyzhao for gpu 2012.11.10
static Int4 s_getVolumeNum(void* seq_src)
{
	TSeqDBData * datap = (TSeqDBData *) seq_src;

	CSeqDBExpert & seqdb = **datap;

	return seqdb.GetVolNum();
}

static Uint8 s_getVolumeLength(void* seq_src, int vol_oid)
{
	TSeqDBData * datap = (TSeqDBData *) seq_src;

	CSeqDBExpert & seqdb = **datap;

	return seqdb.GetVolLength(vol_oid);
}

static void s_getSeqOffsets(void* seq_src, void* offsets_array, int oid)
{
	TSeqDBData * datap = (TSeqDBData *) seq_src;
	...
	seqdb.GetVolSeqOffsets((vector<int>*)offsets_array, oid);
...
#ifdef GPU_RUN
	_BlastSeqSrcImpl_SetGetAllSequence   (retval, & s_SeqDbGetAllSequence);
	_BlastSeqSrcImpl_SetReleaseAllSequence   (retval, & s_SeqDbReleaseSequence);
	_BlastSeqSrcImpl_SetGetVolNum   (retval, & s_getVolumeNum);
	_BlastSeqSrcImpl_SetGetVolLen   (retval, & s_getVolumeLength);
	_BlastSeqSrcImpl_SetGetSeqOffsets   (retval, & s_getSeqOffsets);
#endif
