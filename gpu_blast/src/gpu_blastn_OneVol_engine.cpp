...
#include "../core/blast_gapalign_priv.h"
...
#include "gpu_blastn_seqdb.h"

#include <algo/blast/gpu_blast/gpu_logfile.h>

//////////////////////////////////////////////////////////////////////////
#include <windows.h> // kyzhao for gpu
//////////////////////////////////////////////////////////////////////////
    ...
    BLAST_SequenceBlk* query, BlastCoreAuxStruct** aux_struct_ptr)
{
    ...
    *aux_struct_ptr = aux_struct = (BlastCoreAuxStruct*)
    ...
    if (smith_waterman) {
        ...
        aux_struct->WordFinder = gpu_BlastNaWordFinder;
    }
...
Int4 
BLAST_gpu_PreliminarySearchEngine(EBlastProgramType program_number, 
    BLAST_SequenceBlk* query, BlastQueryInfo* query_info,
    ...
    TInterruptFnPtr interrupt_search, SBlastProgress* progress_info)
{
	...
	int vol_nums = BlastSeqSrcGetVolNum(seq_src);
	seq_arg.oid = 0;

	while(seq_arg.oid < vol_nums) {
		Int4 stat_length;

		if (BlastSeqSrcGetAllSequence(seq_src, &seq_arg) < 0)
			continue;

		vector<int> offset_array;

		BlastSeqSrcGetOffsets(seq_src, &offset_array,seq_arg.oid);
    ...
    while ( (seq_arg.oid = BlastSeqSrcIteratorNext(seq_src, itr)) 
...
#if MULTI_QUERIES
	   SetSubjectID(seq_arg.oid);
	   SetSmallTableSubjectID(seq_arg.oid);
#endif
	  ...
	  //printf("%d, %d\n", seq_arg.seq->oid, seq_arg.seq->length);
#endif
...
#if ONE_VOL
			int num_seqs = offset_array.size();

			vector<BlastHSPList*>  t_hsp_list;

			int nnum_hsp_list = BlastnGPUOneVolOffset(*hsp_list, offset_array, t_hsp_list);	
			for (int i = 0; i< nnum_hsp_list; i++)
			{
			  /* Save the results. */
			status = BlastHSPStreamWrite(hsp_stream, &t_hsp_list[i]);
...
#ifdef PRE_VERSION
		BlastSeqSrcReleaseAllSequence(seq_src, &seq_arg);
#endif
...
#if 0
//多次循环然后把每一个sequence组成一个大的sequence。未完成，需要把每一条sequence都释放，所以这里处理相对麻烦一些。
Int4 
BLAST_gpu_PreliminarySearchEngine_v1(EBlastProgramType program_number, 
    BLAST_SequenceBlk* query, BlastQueryInfo* query_info,
    const BlastSeqSrc* seq_src, BlastGapAlignStruct* gap_align,
    BlastScoringParameters* score_params, 
    LookupTableWrap* lookup_wrap,
...
#if ONE_VOL
			BlastnGPUOneVolOffset(*hsp_list, offset_array);
#endif
			/* Save the results. */
			status = BlastHSPStreamWrite(hsp_stream, &hsp_list);
			if (status != 0)
...
Int2 Blast_gpu_RunPreliminarySearchWithInterrupt(EBlastProgramType program, 
    BLAST_SequenceBlk* query, 
    BlastQueryInfo* query_info, 
    const BlastSeqSrc* seq_src, 
    ...
    TInterruptFnPtr interrupt_search, SBlastProgress* progress_info)
{
...
#if MULTI_QUERIES
#if GPU_SCAN_SMALL
	InitSmallGPUMem(seq_src);
	InitQueryGPUMem(lookup_wrap, query, query_info);
#else
	//InitGPUMem(seq_src,lookup_wrap);
	//InitGPUMemforExtension(lookup_wrap, query);
	//CopyLookupTableTexture(lookup_wrap);
	//CopyExtensionGPUMem(lookup_wrap, query);
	InitGPUMemMultiSeq(seq_src);
	InitGPUMemforExtension(lookup_wrap, query);
#endif

#else
	c1 = slogfile.Start();
	initAllGPUMemoryVolume(seq_src,lookup_wrap, query); 
	c2 = slogfile.End();
	slogfile.addTotalTime("Total Init Memory Time",c1,c2);
#endif
#endif
    ...
    if ((status=
        BLAST_gpu_PreliminarySearchEngine(program, query, query_info, 
...
#else
	c1 = slogfile.Start();
	freeAllGPUMemory();   //gpu_blastn added by kyzhao
	c2 = slogfile.End();
	slogfile.addTotalTime("Total Free GPU Memory Time:", c1, c2);
#endif
#endif
	//slogfile.m_file << "---------------------------------------------\n";
	//slogfile.printTotal();

	slogfile.addTotalNum("lookup_hits", diagnostics->ungapped_stat->lookup_hits, false);
