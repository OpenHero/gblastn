/* $Id: blast_engine.c 345770 2011-11-30 13:58:31Z madden $
* ===========================================================================
*
...
#include "../src/algo/blast/core/blast_gapalign_priv.h"

#include <algo/blast/gpu_blast/gpu_blastn_na_ungapped_v3.h>
...
static BlastCoreAuxStruct* 
	s_BlastCoreAuxStructFree_v3(BlastCoreAuxStruct* aux_struct)
{
...
static BlastCoreAuxStruct* 
	s_gpu_BlastCoreAuxStructFree_v3(BlastCoreAuxStruct* aux_struct)
{
	BlastExtendWordFree(aux_struct->ewp);
	BLAST_InitHitListFree(aux_struct->init_hitlist);
	//sfree(aux_struct->offset_pairs);
...
static void s_BackupSubject_v3(BLAST_SequenceBlk* subject,
...
static void s_AllocateSeqRange_v3(BLAST_SequenceBlk* subject,
	SubjectSplitStruct* backup,
	Int4 num_seq_ranges)
{
...
static void s_RestoreSubject_v3(BLAST_SequenceBlk* subject,
	SubjectSplitStruct* backup)
{
	if (! backup->sequence) return;
...
static const Int2 SUBJECT_SPLIT_DONE = 0;      /**< return value indicating hitting the end */
static const Int2 SUBJECT_SPLIT_OK = 1;        /**< return value indicating OK */
static const Int2 SUBJECT_SPLIT_NO_RANGE = 2;  /**< return value indicating all masked out */

static Int2 s_GetNextSubjectChunk_v3(BLAST_SequenceBlk* subject,
	SubjectSplitStruct *backup,
	Boolean is_nucleotide)
{
	...
	if (subject->mask_type != eSoftSubjMasking) {
		s_AllocateSeqRange_v3(subject, backup, 1);
	...
	s_AllocateSeqRange_v3(subject, backup, len);
...
*/
static void 
	s_TranslateHSPsToDNAPCoord_v3(EBlastProgramType program, 
...
*/
static void
	s_RPSOffsetArrayToContextOffsets_v3(BlastQueryInfo    * info,
	Int4              * new_offsets)
{
...
static Int2
	s_BlastSearchEngineOneContext_v3(EBlastProgramType program_number, 
	...
	SBlastProgress* progress_info)
{
	...
	s_BackupSubject_v3(subject, &backup);

	while (TRUE) {
		status = s_GetNextSubjectChunk_v3(subject, &backup, kNucleotide);
		...
		if (aux_struct->WordFinder) {
			...
			slogfile.addTotalTime("aux_struct->WordFinder Time",c1, c2, false);
		...
		if (score_options->gapped_calculation) {
			...
			if (score_options->is_ooframe) {
				...
				coordinates */
				s_TranslateHSPsToDNAPCoord_v3(program_number, init_hitlist, 
			...
			*/
			...
			__int64 c2 = slogfile.End();
			slogfile.addTotalTime("aux_struct->GetGappedScore",c1, c2, false);
	...
	s_RestoreSubject_v3(subject, &backup);
...
*/
static void
	s_BlastSearchEngineCoreCleanUp_v3(EBlastProgramType program_number,
...
*/
static Int2
	s_BlastSearchEngineCore_v3(EBlastProgramType program_number, 
	...
	SBlastProgress* progress_info)
{
	...
	*hsp_list_out_ptr = NULL;
	...
	if (kTranslatedSubject) {
		...
		if (subject->mask_type != eNoSubjMasking) {
			s_AllocateSeqRange_v3(subject, &backup, backup.num_seq_ranges);
		} else {
	...
	if (Blast_ProgramIsRpsBlast(program_number)) {
		...
		s_RPSOffsetArrayToContextOffsets_v3(query_info, lut->rps_seq_offsets);
	}
	...
	for (context=first_context; context<=last_context; context++) {
		...
		status = s_BlastSearchEngineOneContext_v3(program_number, query, query_info, 
	...
	if (status) {
		...
		s_BlastSearchEngineCoreCleanUp_v3(program_number, query_info, 
	...
	*/
	...
	s_BlastSearchEngineCoreCleanUp_v3(program_number, query_info, query_info_in,
...
*/
static Int2 
	s_FillReturnCutoffsInfo_v3(BlastRawCutoffs* return_cutoffs, 
	const BlastScoringParameters* score_params, 
	const BlastInitialWordParameters* word_params, 
	const BlastExtensionParameters* ext_params,
	const BlastHitSavingParameters* hit_params)
...
*/
static Int2 
	s_BlastSetUpAuxStructures_v3(bool is_gpu,
	...
	BLAST_SequenceBlk* query, BlastCoreAuxStruct** aux_struct_ptr)
{
	...
	if (is_gpu)
	{
		offset_array_size = BlastSeqSrcGetMaxSeqLen(seq_src)/4;
	}
	...
	*aux_struct_ptr = aux_struct = (BlastCoreAuxStruct*)
	...
	if (smith_waterman) {
		...
		aux_struct->WordFinder = gpu_BlastNaWordFinder_v3;
	}
...
*/
static Int2 
	s_RPSPreliminarySearchEngine_v3(EBlastProgramType program_number, 
	BLAST_SequenceBlk* query, BlastQueryInfo* query_info,
	const BlastSeqSrc* seq_src,
	const BlastScoringParameters* score_params, 
	...
	TInterruptFnPtr interrupt_search, SBlastProgress* progress_info)
{
	...
	for (index = 0; index < query_info->num_queries; ++index) {
		...
		status = (Int4)
			s_BlastSearchEngineCore_v3(program_number, &concat_db, one_query_info, 
	...
	if (diagnostics && diagnostics->cutoffs) {
		s_FillReturnCutoffsInfo_v3(diagnostics->cutoffs, score_params, word_params,
...
#include <iostream>
using namespace std;
//由于sequence之间并不是连续的，每一个中间都会有一个间隔，所以，即使按照vol读取，也不能整个读取完整。
//这里记录每一个vol里面每一个sequence的开始的位置，这样就可以记录像对的位置了。

Int4 
	BLAST_gpu_PreliminarySearchEngine_v3(bool is_gpu_thread,
	...
	TInterruptFnPtr interrupt_search, SBlastProgress* progress_info)
{
	...
	//这个参数是在ungaped 或者-gaped参数输入的时候设置的，默认的是要做gapped，就为1.需要参数设置才可以不为1.
	...
	//int i = 0; // for gpu test by kyzhao
	...
	if ((status = 
		s_BlastSetUpAuxStructures_v3(is_gpu_thread,  seq_src, lookup_wrap, word_params, 
		ext_options, hit_options, query, &aux_struct)) != 0)
		return status;

	//cout << "Thread: " << GetCurrentThreadId() << ": aux_struct: " << aux_struct << endl;

	void* org_lp_cb = NULL;
	void* org_lp_et = NULL;// = lookup_wrap->lookup_callback;
	void* new_lp_cb = NULL;
	void* new_lp_et = NULL;// = lookup_wrap->lookup_callback;
	...
	{	
		if (lookup_wrap->lut_type == eSmallNaLookupTable) 
		{
			...
			org_lp_cb = lookup->scansub_callback;
			org_lp_et = lookup->extend_callback;
		}
		else if (lookup_wrap->lut_type == eMBLookupTable) 
		{
			BlastMBLookupTable *mb_lt = (BlastMBLookupTable*)lookup_wrap->lut;
			org_lp_cb = mb_lt->scansub_callback;
			org_lp_et = mb_lt->extend_callback;
		}
			//cout << "Thread: " << GetCurrentThreadId() << "offset_address" << aux_struct->offset_pairs << endl;
			GpuLookUpSetUp(lookup_wrap);
		int num_seq =  BlastSeqSrcGetNumSeqs(seq_src);
		int max_len =  BlastSeqSrcGetMaxSeqLen(seq_src)/4;
		gpu_InitDBMemroy( num_seq, max_len);
		gpu_InitQueryMemory(lookup_wrap, query, query_info);
		
		if (lookup_wrap->lut_type == eSmallNaLookupTable) 
		{
			BlastSmallNaLookupTable *lookup = (BlastSmallNaLookupTable*)lookup_wrap->lut;
			new_lp_cb = lookup->scansub_callback;
			new_lp_et = lookup->extend_callback;
		}
		else if (lookup_wrap->lut_type == eMBLookupTable) 
		{
			BlastMBLookupTable *mb_lt = (BlastMBLookupTable*)lookup_wrap->lut;
			new_lp_cb = mb_lt->scansub_callback;
			new_lp_et = mb_lt->extend_callback;
		}
	}
	...
	if (Blast_ProgramIsRpsBlast(program_number)) {
		status =         
			s_RPSPreliminarySearchEngine_v3(program_number, query, query_info, 
		...
		s_BlastCoreAuxStructFree_v3(aux_struct);
		return status;
	}
	...
	//cout << "max len:" << BlastSeqSrcGetMaxSeqLen(seq_src) << endl;
		...
		!= BLAST_SEQSRC_EOF) {
			...
			{ 			
				...
				{	
					...
					{
						lookup->scansub_callback = org_lp_cb;
						lookup->extend_callback = org_lp_et;
					}
				}
				else if (lookup_wrap->lut_type == eMBLookupTable) 
				{
			...
			status = 
				s_BlastSearchEngineCore_v3(program_number, query, query_info,
...
Int2 Blast_gpu_RunPreliminarySearchWithInterrupt_v3(EBlastProgramType program, 
	BLAST_SequenceBlk* query, 
	BlastQueryInfo* query_info, 
	const BlastSeqSrc* seq_src, 
	...
	TInterruptFnPtr interrupt_search, SBlastProgress* progress_info)
{
	...
	//cout << "Thread: " << GetCurrentThreadId() << ": callback: " << ((BlastSmallNaLookupTable *)lookup_wrap->lut)->scansub_callback << endl;
	...
	if (gpu_id >= 0)
	{
		LookupTableWrap* p_lookup_wrap = new LookupTableWrap(*lookup_wrap);
		lookup_wrap = p_lookup_wrap;
		GpuLookUpInit(lookup_wrap);
		is_gpu_thread = true;
	}
	...
	c2 = slogfile.End();
	slogfile.addTotalTime("PreliminarySearch Time",c1, c2,false);


	if (gpu_id >= 0)
	{
		gpu_ReleaseQueryMemory(lookup_wrap);
		//BlastMGPUUtil.ThreadReplaceGPU();
		delete lookup_wrap->lut;
		delete lookup_wrap;
		lookup_wrap = p_lookup_wrap_back;
	}
	...
	slogfile.addTotalNum("lookup_hits", (long)diagnostics->ungapped_stat->lookup_hits, false);
	slogfile.addTotalNum("init_extends", diagnostics->ungapped_stat->init_extends, false);
	slogfile.addTotalNum("good_init_extends", diagnostics->ungapped_stat->good_init_extends, false);
	slogfile.addTotalNum("extensions", diagnostics->gapped_stat->extensions, false);
	slogfile.addTotalNum("good_extensions", diagnostics->gapped_stat->good_extensions, false);
